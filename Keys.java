...
public class Keys extends JPanel implements KeyListener {
    final int maxLengthOfQuestion = 300;
    public boolean up, down, left, right, interact, talk, teleport, choose, joinButtonClicked, sellBuy, nextDialogue, selected, chestOpen, verify, join;
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (key == KeyEvent.VK_UP) {
            if (gp.ui.slotRow2 != 0) {
                gp.ui.slotRow2 = gp.ui.slotRow2 - 2;
            }
        }
        ...
        if (key == KeyEvent.VK_Q) {
            chestOpen = true;
            if(gp.state == gp.playState) {
                gp.ui.randomName = gp.ui.randomObj();
                gp.repaint();
            }
        }
        //menuUI
