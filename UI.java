...
public class UI {
    ...
    private void cursor(int cursorX, int cursorY, int width, int height, boolean itemBuy) {
        ...
        if (itemBuy) {
            switch (selectedItem) {
                case 12 -> purchaseSellItems(0, 13, true);
                case 13 -> purchaseSellItems(1, 10, true);
                case 14 -> purchaseSellItems(2, 12, true);
                case 15 -> purchaseSellItems(3, 9, true);
    ...
    private void purchaseSellItems(int item, int price, boolean itemBuy) {
        ...
        if (itemBuy) {
            if (keys.sellBuy) {
                switch (item) {
                    case 0 -> transactions(gp.player.coin.image, gp.player.keyObj.image,price, coins);
                    case 1 -> transactions(gp.player.mana.image, gp.player.coin.image, price, manaCoins);
                    case 2 -> transactions(gp.player.coin.image, gp.player.mana.image, price, coins);
                    case 3 -> transactions(gp.player.coin.image, gp.player.diamond.image, price, coins);
                    case 4 -> transactions(gp.player.calcObj.image, gp.player.mana.image, price, manaCoins);
                    case 5 -> transactions(gp.player.lantern.image, gp.player.mana.image, price, manaCoins);
                }
                keys.sellBuy = false;
            }
        }
    ...
    private void transactions(BufferedImage imageMoney, BufferedImage imageItem,  int price, int currency) {
        if(currency >= price){
            int remainingMoney = currency - price;
            int itemCount = gp.player.inventory.getOrDefault(imageItem, 0);
            ...
            if (gp.player.inventory.containsKey(gp.player.lantern.image) ||
                    gp.player.inventory.containsKey(gp.player.calcObj.image)) {
                System.out.println("You already have a lantern or calculator.");
            }
        }
    }
